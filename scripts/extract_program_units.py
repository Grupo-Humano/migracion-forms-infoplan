...
def extract_all_program_units(xml_path, remove_comments=False):
    ...
    for pu in root.findall(".//f:ProgramUnit", NS):
        ...
        pu_type = pu.get("ProgramUnitType") or pu.get(
            "programunittype") or "Unknown"
...
def main():
    ...
    try:
        if specific_pu:
            ...
            result = extract_single_program_unit(
                xml_file, specific_pu, remove_comments)

            if result is None:
                print(
                    f"ERROR: No se encontro el Program Unit '{specific_pu}' en el archivo XML")
                sys.exit(1)
            ...
            program_units = extract_all_program_units(
                xml_file, remove_comments)
            ...
            if output_json:
                ...
                for idx, (pu_name, pu_info) in enumerate(sorted(program_units.items()), 1):
                    ...
                    if len(code_lines) > preview_lines:
                        print(
                            f"... ({len(code_lines) - preview_lines} líneas más)")
                    print()
                ...
                with sql_file.open("w", encoding="utf-8") as f:
                    f.write(
                        f"-- PROGRAM UNITS EXTRAÍDOS DE: {xml_file.name}\n")
