...
def extract_record_groups(root):
    ...
    for rg in root.findall(".//f:RecordGroup", NS):
        ...
        rg_type = rg.get("RecordGroupType") or rg.get("recordgrouptype") or "Query"
        ...
        for col in rg.findall(".//f:ColumnSpecification", NS):
            col_name = col.get("Name") or col.get("name") or ""
            col_type = col.get("ColumnDatatype") or col.get("columndatatype") or ""
            col_length = col.get("MaximumLength") or col.get("maximumlength") or ""
...
def export_to_csv(lovs, record_groups, output_dir: Path, form_name: str):
    ...
    with rg_csv.open("w", newline="", encoding="utf-8") as f:
        fieldnames = ["recordGroupName", "type", "columnCount", "usedByLOVCount", "hasQuery", "queryPreview"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
...
def main():
    ...
    parser.add_argument("--output-dir", "-o", help="Directorio de salida (por defecto: directorio actual)")
    parser.add_argument("--summary", action="store_true", help="Mostrar resumen detallado en consola")
